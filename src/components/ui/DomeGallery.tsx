...
const getDataNumber = (
  ...
  const attr =
    (el as HTMLElement).dataset[name] ?? el.getAttribute(`data-${name}`);
...
export function DomeGallery({
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      if (enlargedOverlay && frameRef.current && mainRef.current) {
        ...
        const el = enlargedOverlay as HTMLElement;

        if (enlargedOverlay.classList.contains("enlarge--preserve-aspect")) {
          const img = enlargedOverlay.querySelector("img");
          if (img && img.naturalWidth > 0) {
            const natW = img.naturalWidth;
            const natH = img.naturalHeight;
            const maxW = mainR.width * 0.85;
            const maxH = mainR.height * 0.85;
            let w = natW;
            let h = natH;
            if (w > maxW || h > maxH) {
          ...
          if (hasCustomSize) {
            ...
            el.style.left = `${frameR.left - mainR.left}px`;
            el.style.top = `${frameR.top - mainR.top}px`;
            el.style.width = `${frameR.width}px`;
            el.style.height = `${frameR.height}px`;
          }
        }
      }
    });
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      const isPreserveAspect = (overlay as HTMLElement).classList.contains(
        "enlarge--preserve-aspect"
      );
      animatingOverlay.className = isPreserveAspect
        ? "enlarge-closing enlarge--preserve-aspect"
        : "enlarge-closing";
      ...
      if (originalImg) {
        ...
        img.style.cssText = `width:100%;height:100%;object-fit:${isPreserveAspect ? "contain" : "cover"};`;
  ...
  const openItemFromElement = useCallback(
    (el: HTMLElement) => {
      ...
      overlay.className = preserveImageAspectRatio
        ? "enlarge enlarge--preserve-aspect"
        : "enlarge";
      ...
      if (preserveImageAspectRatio) {
        const onFirstEnd = (ev: TransitionEvent) => {
          ...
          if (img.complete && img.naturalWidth > 0) {
            applyOriginalRatioSize();
          } else {
            img.onload = () => applyOriginalRatioSize();
          }
        };
  ...
  return (
    <div
      ref={rootRef}
      className={`sphere-root ${className}`}
      style={{
